...
class AssignmentListViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun update(
        ...
        Picasso.get().load(assignment["photo"]).transform(CircleTransform())
            .into(itemView.fragment_assignments_list_item_photo_imageview)

        if (assignment["rating"] != "null") {
            itemView.fragment_assignment_item_list_ratingbar.rating =
                assignment["rating"]!!.toFloat()
        } else {
        ...
        itemView.fragment_assignments_list_item_photo_imageview.setOnClickListener {
            clickListener(
                assignment,
                true
            )
        }
        itemView.fragment_assignments_list_item_click_linearlayout.setOnClickListener {
            clickListener(
                assignment,
                false
            )

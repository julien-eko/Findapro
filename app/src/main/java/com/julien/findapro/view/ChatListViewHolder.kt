...
class ChatListViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun update(
        ...
        Picasso.get().load(chat["photo"].toString()).transform(CircleTransform())
            .into(itemView.fragment_chat_list_item_photo_imageview)

        itemView.fragment_chat_list_item_clik_linearlayout.setOnClickListener {
            clickListener(
                chat,
                false
            )
        }
        itemView.fragment_chat_list_item_photo_imageview.setOnClickListener {
            clickListener(
                chat,
                true
            )

...
class ProfilViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun update(
        profilRate: HashMap<String, String>,
        ...
        Picasso.get().load(profilRate["photo"]).transform(CircleTransform())
            .into(itemView.activity_profil_item_photo_imageview)

        itemView.activity_profil_item_photo_imageview.setOnClickListener {
            clickListener(
                profilRate,
                true
            )
